...
int dustPin=39;      //gpio39接灰塵偵測器
int dustLED=12;      //gpio12接灰塵偵測器內部LED
float dustVal=0;     //設定灰塵讀取初始值=0
int delayTime=280;
int delayTime2=40;
float offTime=9680;
...
char* pubTopic4="hhvs/library/plant/sharpSensor"; //推播灰塵值
...
void setup()
{
  ...
  pinMode(dustPin,INPUT);       //設定為讀取接腳(讀取Sharp sensor值)
  pinMode(dustLED,OUTPUT);      //設定為輸出接腳(開啟sharp sensor偵測用的LED)
...
void loop()
{
  ...
  dustVal=analogRead(dustPin);      //讀取感測器灰塵值
  int Dust_Value=(float(dustVal/1024)-0.0356)*120000*0.035 ; //轉換讀取值
  ...
  if((millis() - MQTTLastPublishTime) >= MQTTPublishInterval)  //若傳輸時間超過前面設定的10秒(MQTTPublishInterval，則繼續發佈到mqtt
  {
    ...
    MQTTClient.publish(pubTopic4,String(Dust_Value).c_str());  //推播灰塵值
